...
final class StageRenderer {
    ...
    func render(
        ...
        case .single:
            hasAnimation = liveVideo != nil
                ? buildLivePhoto(stage, current: current, videoFile: liveVideo!, photoTimeMs: photoTimeMs)
                : buildSingle(stage, current: current, intervalMs: intervalMs, photoTimeMs: photoTimeMs)
        case .kenBurns:
            hasAnimation = liveVideo != nil
                ? buildLivePhoto(stage, current: current, videoFile: liveVideo!, photoTimeMs: photoTimeMs)
                : buildKenBurns(stage, current: current, intervalMs: intervalMs, photoTimeMs: photoTimeMs)
        case .polaroid:
            buildPolaroid(stage, current: current, caption: caption)
            hasAnimation = false
        case .collage:
            buildCollage(stage, current: current, upcoming: upcomingFiles)
            hasAnimation = false
        case .splitScreen:
            buildSplitScreen(stage, current: current, upcoming: uniqueUpcoming,
                             intervalMs: intervalMs, photoTimeMs: photoTimeMs)
            hasAnimation = false
        case .filmstrip:
            buildFilmstrip(stage, current: current, upcoming: upcomingFiles)
            hasAnimation = false
        case .mosaic:
            buildMosaic(stage, current: current, upcoming: upcomingFiles)
            hasAnimation = false
        case .cinematic:
            hasAnimation = buildCinematic(stage, current: current,
                                          intervalMs: intervalMs, photoTimeMs: photoTimeMs)
        case .memories:
            hasAnimation = buildMemories(stage, current: current, upcoming: upcomingFiles,
                                         intervalMs: intervalMs, photoTimeMs: photoTimeMs)
        case .random:
    ...
    private func buildLivePhoto(stage: UIView, current: URL, videoFile: URL, photoTimeMs: Int64) -> Bool {
        // 1. 静态封面
        let cover = makeImageView(stage)
        loadInto(cover, current)
    ...
    private func buildMemories(
        ...
        if choice < 0.10, upcoming.count >= 2 {
            lastConsumedCount = 3
            buildMemoriesCollage(stage, current: current, upcoming: upcoming)
            return false
        }
        ...
        if cinematic {
            addMemoriesCinematicBars(stage)
        }
